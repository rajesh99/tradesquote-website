...
description: "How an HVAC load calculation — also called an air conditioning load or AC load calculation — works: the heat-loss formula Q = U × A × ΔT, the sensible and latent parts of a cooling load, a worked residential example, and how to convert load to tonnage."
...
A cooling load calculation also goes by different names depending on who's asking — an **air conditioning load calculation**, **air conditioner load calculation**, **AC load calculation**, or **aircon load calculation** (the common term in the UK, Australia, and much of Southeast Asia) all describe this exact same sensible-plus-latent math. The terminology changes by region and trade; the physics and the formula below do not.

## The Heat-Loss Formula

At its core, calculating heat loads for any building comes down to a single formula applied to every surface:
...
### Is an air conditioner load calculation different from a cooling load calculation?

No. Air conditioning load calculation, AC load calculation, and aircon load calculation are regional names for the same cooling-load process — sensible heat gain plus latent heat gain, sized from the building's actual construction rather than a square-footage guess. Whichever term your contractor or software uses, the math and the result are identical.
