...
But those are starting points, not answers. Feed your own numbers into the **[Water Pipe Size Calculator](/calculators/plumbing/plumbing-pipe-size-calculator)** and it will tell you which of the two rules governed — which is the part that matters when someone questions the size.
...
Notice the shape of that result. The pipe that fails velocity also fails pressure, and the pipe that clears velocity clears pressure with room to spare. That is common on short residential runs, and it is exactly why a velocity-only chart usually gets away with it. Stretch the run to 250 ft or drop the street pressure to 45 psi and the two rules come apart — friction starts governing while velocity still looks fine.
...
A two-bath house with a kitchen sink, dishwasher, clothes washer and two hose bibbs comes to **16.4 WSFU**, which the curve turns into about **18.2 gpm** on a flush-tank system. Run it yourself with the **[Water Supply Fixture Unit Calculator](/calculators/plumbing/plumbing-water-supply-fixture-unit-calculator)**.
...
<BlogInfographic
  ...
  caption="Across ordinary residential flows on a 120 ft run, velocity governs every rung. Lengthen the run or drop the supply pressure and friction takes over."
>
...
**Bore beats smoothness, every time.** The practical rule: when you substitute PEX for copper on a sized drawing, go up a nominal size on the mains and long branches. Short fixture drops are usually fine as-is. Check any specific case with the **[Pipe Velocity Calculator](/calculators/plumbing/plumbing-pipe-velocity-calculator)**.
