...
import FurnaceClimateZoneDiagram from "@/components/blog/furnace-sizing/FurnaceClimateZoneDiagram.astro";
import AfueConversionDiagram from "@/components/blog/furnace-sizing/AfueConversionDiagram.astro";
import OversizedFurnaceCycleDiagram from "@/components/blog/furnace-sizing/OversizedFurnaceCycleDiagram.astro";
...
HVAC engineers size furnaces using a heat loss calculation (Manual J), but the BTU-per-square-foot shortcut gets you close enough to verify a contractor's quote or narrow down your options. This is the heating-side counterpart to cooling capacity — if you also need to size an air conditioner, see **[how many BTU you need for AC](/blog/how-many-btu-do-i-need)**.
...
<BlogInfographic
  title="Furnace BTU per square foot by climate zone"
  caption="Zones 1–2 use 30, zones 3–4 use 45, and zones 5–6 use 60 BTU per square foot."
>
  <FurnaceClimateZoneDiagram />
</BlogInfographic>
...
<BlogInfographic
  title="AFUE input vs output BTU"
  caption="Divide required output BTU by AFUE to find the correct nameplate input size."
>
  <AfueConversionDiagram />
</BlogInfographic>
...
The fix is not always a smaller furnace — it's an accurate load calculation. If a contractor quotes you a 120k unit for a 2,200 sq ft Denver home without showing you a Manual J calculation, ask for one. Our comparison of **[Manual J vs. rules of thumb](/blog/manual-j-vs-rule-of-thumb)** explains why the per-square-foot figure is only a sanity check and the load calc is what should size the equipment.

<BlogInfographic
  title="Oversized furnace consequences"
  caption="Short-cycling creates cold spots, higher bills, and accelerated heat exchanger wear."
>
  <OversizedFurnaceCycleDiagram />
</BlogInfographic>
...
Two-stage makes the most sense when you have a large home or rooms that are hard to heat evenly. For a smaller, well-insulated home, the price premium may not be worth it. Once you have settled on a size, our **[furnace replacement cost breakdown](/blog/furnace-replacement-cost)** shows what that nameplate translates to in installed dollars. And if you live somewhere with hard winters, it is worth weighing a furnace against a **[cold-climate heat pump](/blog/heat-pump-sizing-cold-climates)** before you buy.
