...
**Knowing when not to speed up the blower.** If a system is starved for airflow because static pressure is too high, the cube law says brute force is the costly fix. Speeding up the blower to push through a restriction burns power fast. Removing the restriction — larger return ducts, a cleaner filter, a clean coil, fewer flex bends — lets the blower deliver the same air at lower speed and far lower power. Verify the static problem with the **[Static Pressure Calculator](/calculators/hvac/hvac-static-pressure-calculator)** before reaching for a faster blower.
